let
    ...
    env = construct_search_env(solver,project_spec, problem_spec, robot_ICs, assignments, env_graph)
    pc_mapf = PC_MAPF(env)
...
let
    ...
    high_level_search!(PC_TAPF_Solver(nbs_model=AssignmentMILP()),env_graph,project_spec,problem_spec,robot_ICs,Gurobi.Optimizer);
    high_level_search!(PC_TAPF_Solver(nbs_model=SparseAdjacencyMILP()),env_graph,project_spec,problem_spec,robot_ICs,Gurobi.Optimizer);
end
