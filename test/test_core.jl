...
let
    ...
    project_spec = ProjectSpec(initial_conditions=object_ICs)
    add_operation!(project_spec,construct_operation(5, [1,2], [3], 1.0))
    add_operation!(project_spec,construct_operation(6, [3], [], 0.0))
    assignments = [1,2,3]
...
#     project_spec = construct_random_project_spec(M,object_ICs;max_parents=3,depth_bias=1.0,Δt_min=0,Δt_max=0)
#     object_ICs1 = Dict{Int,OBJECT_AT}(o => object_ICs[o] for o in 1:Int(M/2))
#     project_spec1 = construct_random_project_spec(Int(M/2),object_ICs1;max_parents=3,depth_bias=0.25,Δt_min=0,Δt_max=0)
#     object_ICs2 = Dict{Int,OBJECT_AT}(o => object_ICs[o] for o in Int(M/2):M)
#     project_spec2 = construct_random_project_spec(Int(M/2),object_ICs2;max_parents=3,depth_bias=0.25,Δt_min=0,Δt_max=0)
